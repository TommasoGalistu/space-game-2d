...
/* eslint-disable */
(_console = console).log.apply(_console, _toConsumableArray(oo_oo("4279877021_32_0_32_35_4", array1, array2, array3)));
...
function animate() {
  ...
  platforms.forEach(function (platform) {
    if (player.position.y + player.height <= platform.position.y && player.position.y + player.height + player.velocity.y >= platform.position.y && player.position.x + player.width >= platform.position.x && player.position.x <= platform.position.x + platform.width) {
      player.velocity.y = 0;
      obstacles.forEach(function (obstacle) {
        if (player.position.x + player.width + player.velocity.x >= obstacle.position.x && player.position.x - player.velocity.x <= obstacle.position.x + obstacle.width && player.position.y + player.height >= obstacle.position.y) {
          player.velocity.x = 0;
        }
      });
    } else {
  ...
  if (scrollOffset > 7640) {
    var _console2;
    /* eslint-disable */(_console2 = console).log.apply(_console2, _toConsumableArray(oo_oo("4279877021_326_8_326_30_4", 'you win')));
  }
